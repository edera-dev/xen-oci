...
./configure --prefix=/usr --enable-xen --target-list=${target}-softmmu \
...
for PROG in elf2dmp qemu-edid qemu-ga qemu-img qemu-io qemu-nbd qemu-pr-helper qemu-storage-daemon qemu-system-${target} qemu-vmsr-helper; do
  ...
  [ -x "/usr/src/qemu-xen/output/usr/bin/$PROG" ] && patchelf --set-interpreter /opt/edera/lib/ld-musl-$(apk --print-arch).so.1 --add-rpath /opt/edera/lib --add-rpath /opt/edera/usr/lib "/usr/src/qemu-xen/output/usr/bin/$PROG";
done
